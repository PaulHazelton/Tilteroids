...
public class BloomFilter : IDisposable
{
	...
	public BloomFilter(Effect bloomEffect, GraphicsDevice graphicsDevice, int width, int height, SurfaceFormat renderTargetFormat = SurfaceFormat.Color)
	{
	...
	{
		// Store old render target to avoid side effects
		...
		if (width != _width || height != _height)
		{
			...
			// Adjust the blur so it looks consistent across different scalings
	...
	/// <summary>
	/// Update the InverseResolution of the used render targets. This should be the InverseResolution of the processed image
	...
	/// <summary>
	/// Renders a simple quad to the screen. Uncomment the Vertex / Index buffers to make it a static full screen quad.
